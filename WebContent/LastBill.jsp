...
<body id="page-top" data-spy="scroll" data-target=".navbar-custom" style="background:url('images/bg1.jpg') no-repeat;">
    ...
    %>
<center><h2>Bill History of <%=pname%>&nbsp;&nbsp;[<%=pid%>] </h2></center>
<table id="myTable" class="table table-striped table-bordered" >
    <thead>
        <tr>
            <th>Sr. No.</th><th>Doctor Name</th><th>Department</th><th>Appointment Date</th><th>Bill Amount</th><th>Bill Paid Date</th>
        </tr>
    </thead>
    <tbody>
        <%
            ...
            try {
                ...
                st = con.createStatement();//select a.*,b.* from appointment a join patient_bill b on a.pid=b.pid and a.flag=1 and a.pid='pid-104' group by a.dname
                rs = st.executeQuery("select distinct a.dname,a.appoint_date_time,a.department,a.fees,b.date from appointment a, patient_bill b where a.flag=1 and a.pid=b.pid and a.pid='" + pid + "' and b.bill=a.fees");
                while (rs.next()) {
                    count++;
        %>
        <tr>
            <td><%=count%></td><td><%=rs.getString("dname")%></td><td><%=rs.getString("department")%></td><td><%=rs.getString("appoint_date_time")%></td><td><%=rs.getString("fees")%></td><td><%=rs.getString("date")%></td>
        </tr>
        <%
                }
            } catch (Exception e) {
