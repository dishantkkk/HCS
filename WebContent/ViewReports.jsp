...
<table id="myTable" class="table table-striped table-bordered" >
    ...
    <tbody>
        ...
        %>
        <tr>
            <td><%=count%></td><td><a title="click to view report" href="reports.jsp?_tokens=<%=tokens%>&id=<%=rs.getString("pid")%>&tokens=<%=tokens%>&name=<%=rs.getString("Pname")%>&tokens=<%=tokens%>&tokens=<%=tokens%>&tokens=<%=tokens%>"><%=rs.getString("pid")%></a></td><td><%=rs.getString("Pname")%></td>
        </tr>
        <%
                }
            } catch (Exception e) {
